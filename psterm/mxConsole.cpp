...
void mxConsole::OnPaint (wxPaintEvent & event) {
	...
	if (dimmed) { status[0]=_Z("El algoritmo fue modificado."); status[1]=_Z("Click aqu� para aplicar los cambios."); }
	else if (selection_is_input) { status[0]=_Z("Utilice doble click para"); status[1]=_Z("modificar solo esa lectura."); }
	else if ( (want_input || wait_one_key) && cur_loc.IsValid()) status[1]<<_Z("l�nea ")<<cur_loc.line<<_Z(" instrucci�n ")<<cur_loc.inst;
	if (status[1].Len()) {
...
//	menu.Append(CONSOLE_ID_POPUP_RESET,"Reiniciar ejecuci�n");
//	menu.Append(CONSOLE_ID_POPUP_FROM_HERE,"Continuar desde aqu�");
	...
	menu.AppendCheckItem(CONSOLE_ID_POPUP_CLOSE_AFTER_RUN,_Z("Cerrar al finalizar la ejecuci�n"))->Check(!parent->GetDoNotClose());
	PopupMenu(&menu);
}
...
void mxConsole::GetSourceLocationFromOutput (int pos) {
	...
	// mostrar seleccion y marcar en el pseudoc�digo la instrucci�n correspondiente
	Refresh();
