		...
		switch (inst.type) {
			case IT_FINPROCESO: {
				...
				if (inst_impl.principal) {
					_sub(line,LocalizationManager::Instance().Translate("Finaliza el algoritmo"));
				} else {
					_sub(line,string(LocalizationManager::Instance().Translate("Se sale del subproceso "))+getImpl<IT_FINPROCESO>(inst).nombre);
					Inter.OnFunctionOut();
				}
			} return;
			case IT_PROCESO: {
				...
				_pos(line);
				_sub(line,string(inst_impl.principal?LocalizationManager::Instance().Translate("El algoritmo comienza con el proceso "):LocalizationManager::Instance().Translate("Se ingresa en el subproceso "))+inst_impl.nombre);
			} break;
			case IT_BORRARPANTALLA: {
				...
				_sub(line,LocalizationManager::Instance().Translate("Se borra la pantalla"));
			} break;
			case IT_ESPERARTECLA: {
				_pos(line);
				_sub_msg(line,LocalizationManager::Instance().Translate("Se espera a que el usuario presione una tecla."));
				_sub_raise();
			...
			case IT_INVOCAR: {
				...
				_sub(line,string(LocalizationManager::Instance().Translate("Se va a invocar al subproceso"))+inst_impl.nombre);
			...
			case IT_ESCRIBIR: {
				...
				for(size_t i_expr=0;i_expr<inst_impl.expresiones.size();++i_expr) {
					...
					_sub(line,string(LocalizationManager::Instance().Translate("Se evalua la expresion: "))+expression);
					...
					if (res.IsOk()) {
						...
						_sub(line,string(LocalizationManager::Instance().Translate("Se muestra en pantalla el resultado: "))+res.GetForUser());
					}
				}
			...
			case IT_LEER: {
				...
				for(size_t i_var = 0; i_var<inst_impl.variables.size(); ++i_var) {
					...
					if (auxup==LocalizationManager::Instance().Translate("VERDADERO") || auxup==LocalizationManager::Instance().Translate("FALSO")) aux1=auxup;
			...
			case IT_DIMENSION: {
				...
				for(size_t i=0;i<inst_impl.nombres.size();++i) { 
					...
					if (lang[LS_ALLOW_DINAMYC_DIMENSIONS]) { _sub(line,string(LocalizationManager::Instance().Translate("Se evaluan las expresiones para cada dimension del arreglo "))+nombre); }
