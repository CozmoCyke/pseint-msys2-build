...
DataValue func_rc(ErrorHandler &err_handler, DataValue *arg) {
	...
	if (x < 0) {
		err_handler.ErrorIfRunning(147, LocalizationManager::Instance().Translate(LocalizationManager::Instance().Translate("Raiz cuadrada de numero negativo.")));
...
DataValue func_asen(ErrorHandler &err_handler, DataValue *arg) {
	double x = arg[0].GetAsReal();
	if (x < -1 || x > +1) {
		err_handler.ErrorIfRunning(312, LocalizationManager::Instance().Translate(LocalizationManager::Instance().Translate("Argumento invalido para la funcion ASEN (debe estar en [-1;+1]).")));
		return DataValue::MakeEmpty(vt_numerica);
	}
...
DataValue func_acos(ErrorHandler &err_handler, DataValue *arg) {
	double x = arg[0].GetAsReal();
	if (x < -1 || x > +1) {
		err_handler.ErrorIfRunning(312, LocalizationManager::Instance().Translate(LocalizationManager::Instance().Translate("Argumento invalido para la funcion ACOS (debe estar en [-1;+1]).")));
		return DataValue::MakeEmpty(vt_numerica);
	}
