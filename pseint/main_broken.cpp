...
int main(int argc, char* argv[]) {
	...
	int 
		forced_seed=-1; // semilla a utilizar para inicializar la generacin de nmeros aleatorios (si es -1 se usa el reloj)
...
";
		...
		cout<<"      --input=<str>          sirve para predefinir uno o ms valores de entrada para acciones LEER"<<endl;
		cout<<"      --fixwincharset        corrige la codificacin de algunos caracteres para que se muestren correctamente"<<endl;
		...
		cout<<"                             numeros de linea e instruccin necesarios para marcar la ejecucin paso a paso"<<endl;
		...
		cout<<"      --profile=<archivo>    archivo de perfil a utilizar para configurar el intrprete"<<endl;
...
//			memoria->HardReset();
	...
	if (!real_time_syntax) { SynCheck(rt); }
...
//				CaseMapPurge();
			...
			if (user) show_user_info("*** Ejecucin Iniciada. ***");
			Ejecutar(rt,main_func->line_start);
			Inter.SetFinished();
			if (ExeInfoOn) ExeInfo<<LocalizationManager::Instance().Translate("*** Ejecucion Finalizada. ***");
			if (user) show_user_info("*** Ejecucin Finalizada. ***");
		}
	} else {
		if (user) cout<<endl;
		int errores = rt.errGetErrorsCount();
		if (errores==1) {
