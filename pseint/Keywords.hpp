...
struct Keyword {
	...
	std::vector<std::string> visible_alternatives; // formas visibles/originales
	...
	void clear() { alternatives.clear(); visible_alternatives.clear(); preferred.clear(); }
	bool IsOk() const {
		_expects(alternatives.empty()==preferred.empty());
		_expects(alternatives.size()==visible_alternatives.size());
