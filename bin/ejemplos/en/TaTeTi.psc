...
{Proceso} TaTeTi
	...
	// the other it is 2, so to determine who won you multiply by row, by 
	// column, and by diagonal; if the result is 1 the first player won,
	...
	Mientras ~ Terminado hacer
		...
		Si ~ HayGanador y CantTurnos<9 Entonces
			...
			// check whether someone won by looking for the target product on the board
			aux_d1 <- 1; aux_d2 <- 1{;}
			Para i <- {1#0} hasta {3#2} hacer
				aux_i <- 1; aux_j <- 1{;}
