...
static void AddCalltipEntry(std::vector<calltip_text> &target, const wxString &key_raw, const wxString &text, bool only_if_not_first=false) {
	...
	for (const auto &item : target)
		if (item.key==key && item.text==text && item.only_if_not_first==only_if_not_first)
			return;
	target.push_back(calltip_text(key,text,only_if_not_first));
}

static void AddCalltipKeywordForms(std::vector<calltip_text> &target, KeywordType kw, const wxString &text, bool only_if_not_first, std::initializer_list<const char*> forms) {
	for (const char *form : forms)
		AddCalltipEntry(target, GetKeywordVisibleForm(kw, form), text, only_if_not_first);
}

static void AddCompletionEntry(const wxString &label, const wxString &text, const wxString &instruction) {
	wxString instruction_key = instruction.Lower();
	for (const auto &item : comp_list)
		if (item.label==label && item.text==text && item.instruction==instruction_key)
			return;
	comp_list.push_back(comp_list_item(label,text,instruction));
}

static void AddCompletionKeywordForms(KeywordType kw, const wxString &suffix, const wxString &instruction, std::initializer_list<const char*> forms) {
	for (const char *form : forms) {
		wxString label = GetKeywordVisibleForm(kw, form);
...
void mxSource::SetAutocompletion() {
	...
	AddCompletionKeywordForms(KW_ALGORITMO, " ", "", {"Proceso"});
	if (cfg_lang[LS_ENABLE_USER_FUNCTIONS]) {
		AddCompletionKeywordForms(KW_ALGORITMO, " ", "", {"Algoritmo"});
		AddCompletionKeywordForms(KW_SUBALGORITMO, " ", "", {"Funcion", "SubAlgoritmo", "SubProceso"});
		AddCompletionKeywordForms(KW_POR_COPIA, "", GetKeywordVisibleForm(KW_SUBALGORITMO, "SubAlgoritmo"), {"Por Valor"});
		AddCompletionKeywordForms(KW_POR_COPIA, "", GetKeywordVisibleForm(KW_SUBALGORITMO, "SubProceso"), {"Por Valor"});
		AddCompletionKeywordForms(KW_POR_COPIA, "", GetKeywordVisibleForm(KW_SUBALGORITMO, "Funcion"), {"Por Valor"});
		AddCompletionKeywordForms(KW_POR_REFERENCIA, "", GetKeywordVisibleForm(KW_SUBALGORITMO, "SubAlgoritmo"), {"Por Referencia"});
		AddCompletionKeywordForms(KW_POR_REFERENCIA, "", GetKeywordVisibleForm(KW_SUBALGORITMO, "SubProceso"), {"Por Referencia"});
		AddCompletionKeywordForms(KW_POR_REFERENCIA, "", GetKeywordVisibleForm(KW_SUBALGORITMO, "Funcion"), {"Por Referencia"});
	}
	AddCompletionKeywordForms(KW_FINALGORITMO, "\n", "", {"FinProceso", "FinAlgoritmo"});
	if (cfg_lang[LS_LAZY_SYNTAX])
		AddCompletionKeywordForms(KW_FINALGORITMO, "\n", "", {"Fin Proceso", "Fin Algoritmo"});
	if (cfg_lang[LS_ENABLE_USER_FUNCTIONS]) {
		AddCompletionKeywordForms(KW_FINSUBALGORITMO, "\n", "", {"FinSubProceso", "FinSubAlgoritmo", "FinFuncion"});
		if (cfg_lang[LS_LAZY_SYNTAX])
			AddCompletionKeywordForms(KW_FINSUBALGORITMO, "\n", "", {"Fin SubAlgoritmo", "Fin Funcion", "Fin SubProceso"});
	}
	
	AddCompletionKeywordForms(KW_ESCRIBIR, " ", "", {"Escribir"});
	if (cfg_lang[LS_LAZY_SYNTAX])
		AddCompletionKeywordForms(KW_ESCRIBIR, " ", "", {"Imprimir", "Mostrar"});
	AddCompletionKeywordForms(KW_SIN_SALTAR, "", GetKeywordVisibleForm(KW_ESCRIBIR, "Escribir"), {"Sin Saltar"});
	AddCompletionKeywordForms(KW_SIN_SALTAR, "", GetKeywordVisibleForm(KW_ESCRIBIR, "Mostrar"), {"Sin Saltar"});
	AddCompletionKeywordForms(KW_SIN_SALTAR, "", GetKeywordVisibleForm(KW_ESCRIBIR, "Imprimir"), {"Sin Saltar"});
	AddCompletionKeywordForms(KW_LEER, " ", "", {"Leer"});
	
	AddCompletionKeywordForms(KW_ESPERARTIEMPO, " ", "", {"Esperar"});
	AddCompletionKeywordForms(KW_SEGUNDOS, ";", GetKeywordVisibleForm(KW_ESPERARTIEMPO, "Esperar"), {"Segundos"});
	AddCompletionKeywordForms(KW_MILISEGUNDOS, ";", GetKeywordVisibleForm(KW_ESPERARTIEMPO, "Esperar"), {"Milisegundos"});
	AddCompletionEntry("Tecla", "Tecla;", GetKeywordVisibleForm(KW_ESPERARTIEMPO, "Esperar"));
	AddCompletionKeywordForms(KW_ESPERARTECLA, ";", "", {"Esperar Tecla"});
	AddCompletionKeywordForms(KW_LIMPIARPANTALLA, ";", "", {"Borrar Pantalla", "Limpiar Pantalla"});
	
	// AddCompletionEntry("Dimension","Dimension ","");
	AddCompletionKeywordForms(KW_DIMENSIONAR, " ", "", {"Dimensionar"});
	if (cfg_lang[LS_ALLOW_RESIZE_ARRAYS]) {
		// AddCompletionEntry("Redimension","Redimension ","");
	...
	AddCompletionKeywordForms(KW_SINO, "\n", "", {"SiNo"});
	AddCompletionKeywordForms(KW_FINSI, "\n", "", {"FinSi"});
	if (cfg_lang[LS_LAZY_SYNTAX]) AddCompletionKeywordForms(KW_FINSI, "\n", "", {"Fin Si"});
	
	AddCompletionKeywordForms(KW_MIENTRAS, " ", "", {"Mientras"});
	AddCompletionKeywordForms(KW_HACER, "\n", GetKeywordVisibleForm(KW_MIENTRAS, "Mientras"), {"Hacer"});
	AddCompletionKeywordForms(KW_FINMIENTRAS, "\n", "", {"FinMientras"});
	if (cfg_lang[LS_LAZY_SYNTAX]) AddCompletionKeywordForms(KW_FINMIENTRAS, "\n", "", {"Fin Mientras"});
	
	AddCompletionKeywordForms(KW_PARA, " ", "", {"Para"});
	if (cfg_lang[LS_ALLOW_FOR_EACH])
		AddCompletionKeywordForms(KW_PARACADA, " ", "", {"Para Cada"});
	if (cfg_lang[LS_LAZY_SYNTAX])
		AddCompletionKeywordForms(KW_DESDE, " ", GetKeywordVisibleForm(KW_PARA, "Para"), {"Desde"});
	AddCompletionKeywordForms(KW_HASTA, " ", GetKeywordVisibleForm(KW_PARA, "Para"), {"Hasta"});
	AddCompletionKeywordForms(KW_CONPASO, " ", GetKeywordVisibleForm(KW_PARA, "Para"), {"Con Paso"});
	AddCompletionKeywordForms(KW_HACER, "\n", GetKeywordVisibleForm(KW_PARA, "Para"), {"Hacer"});
	if (cfg_lang[LS_ALLOW_FOR_EACH]) AddCompletionEntry("Cada ", "Cada ", GetKeywordVisibleForm(KW_PARA, "Para"));
	AddCompletionKeywordForms(KW_FINPARA, "\n", "", {"FinPara"});
	if (cfg_lang[LS_LAZY_SYNTAX]) AddCompletionKeywordForms(KW_FINPARA, "\n", "", {"Fin Para"});
	
	AddCompletionKeywordForms(KW_REPETIR, "\n", "", {"Repetir"});
	AddCompletionKeywordForms(KW_HACER, "\n", "", {"Hacer"});
	AddCompletionKeywordForms(KW_HASTAQUE, " ", "", {"Hasta Que"});
	if (cfg_lang[LS_ALLOW_REPEAT_WHILE]) AddCompletionKeywordForms(KW_MIENTRASQUE, " ", "", {"Mientras Que"});
	
	AddCompletionKeywordForms(KW_SEGUN, " ", "", {"Segun"});
	if (cfg_lang[LS_LAZY_SYNTAX])
		AddCompletionKeywordForms(KW_OPCION, " ", "", {"Opcion", "Caso"});
	AddCompletionEntry(GetKeywordVisibleForm(KW_DEOTROMODO, "De Otro Modo")+":", GetKeywordVisibleForm(KW_DEOTROMODO, "De Otro Modo")+":\n", "");
...
void mxSource::RTOuputEnds ( ) {
	...
	Update(); // sin esto revienta extra�amente luego al hacer un update del aui desde el QuickHelpPanelPolicy...
	          ...
	          // intentar reproducirlo el comportamiento es raro.. podr�a ser undefined behavior (bug de wx?)
...
bool mxSource::IsDimOrDef(int line) {
	...
	if (cfg_lang[LS_ALLOW_RESIZE_ARRAYS]) {
		...
		if (fword=="REDIMENSI�N ") return true;
...
static void FixExtraUnicode_impl(wxString &s, int i0, int iN) {
	...
	const auto data = cs.cWX2MB(s.Mid(i0,iN-i0).wc_str());
...
bool mxSource::SaveFile (const wxString & fname) {
	...
	static wxCSConv cs("ISO-8859-1");
	const auto data = cs.cWX2MB(s.wc_str());
	bool write_ok = data;
	if (data) {
