...
void mxConfig::OnSaveButton (wxCommandEvent & evt) {
	...
	if (dlg.ShowModal() == wxID_OK) {
		...
		LangSettings l(LS_INIT);
		while(true) {
//			l.descripcion = _W2S( wxGetTextFromUser(_Z("Ingrese una descripci�n del perfil (\n"
//									   		           "incluya materia, carrera, instituci�n\n"
//											           "y nombre del docente)."),_Z("Guardar Perfil"),"",this) );
			
			wxTextEntryDialog dialog(this, 
									 _Z("Ingrese una descripci�n del perfil (incluya materia,\n"
										"carrera, instituci�n y nombre del docente)."),
