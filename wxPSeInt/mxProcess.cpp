...
#ifdef __APPLE__
	// El redirect que sigue en teor�a no deber�a ser necesario... pero sin esto
	// no funciona el wxExecute en Mac OS 10.13 en adelante con wx 2.8
	// Tal vez pueda sacarlo cuando migre a wx3??
	: wxProcess(wxPROCESS_REDIRECT) 
	// por otro lado, parece que a linux le es indiferente, pero en windows s�
	// molesta el redirect si no corresponde (por alguna extra�a raz�n hay que 
...
static void Execute(const wxString &command, wxArrayString &output) {
	...
	// porque en ese caso no se puede controlar la codificaci�n de la
...
bool mxProcess::CheckSyntax(wxString file, wxString extra_args) {
	...
	if (errors_count!=0) {
		...
		main_window->RTreeAdd("Las lineas con errores se marcan con una cruz sobre el margen izquierdo. Seleccione un error para ver su descripci�n:",mxMainWindow::RTAddType::OutOfTree);
...
bool mxProcess::ExportLang(wxString file, wxString lang, bool check_first) {
	...
	if (check_first) return CheckSyntax(file,wxString("--export --preservecomments z\"")<<source->GetTempFilenamePSD()<<"\"");
